"""Durable serial worker for uploaded human-narrated audiobooks."""
...
from .audiobook_import import process_import

logger = logging.getLogger(__name__)


class AudiobookImportQueue:
    ...
    async def start(self) -> None:
        if self._worker_task and not self._worker_task.done():
            return
        self._worker_task = asyncio.create_task(self._run(), name="audiobook-import-worker")
    ...
    async def requeue_pending(self) -> int:
        async with SessionLocal() as db:
            result = await db.execute(
                select(ImportedAudiobook.id).where(ImportedAudiobook.status.in_(("queued", "importing")))
            )
        ...
        while True:
            ...
            try:
                if edition_id is None:
                    return
                async with SessionLocal() as db:
                    await process_import(edition_id, db)
            except Exception:
                logger.exception("Unhandled audiobook import failure for edition %s.", edition_id)
            finally:
...
_queue: AudiobookImportQueue | None = None


def get_audiobook_import_queue() -> AudiobookImportQueue:
    global _queue
    if _queue is None:
        _queue = AudiobookImportQueue()
    return _queue
