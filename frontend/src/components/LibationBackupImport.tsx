...
type BackupPreview = components["schemas"]["LibationBackupPreviewResponse"];
type BackupMatch = components["schemas"]["LibationBackupMatchResponse"];
type BookOption = components["schemas"]["LibationBookOptionResponse"];
interface MatchReview {
  bookId: number | null;
  included: boolean;
  input: string;
}
type ImportResult = BackupMatch & {
...
function libationSourceKey(pathValue: string) {
...
function filesBySourceKey(files: File[]) {
...
function statusLabel(group: BackupMatch, existingAudioCount = 0) {
  switch (group.status) {
    case "matched":
...
function bookOptionLabel(book: BookOption | BackupMatch) {
...
function audioStatusLabel(status: string | null) {
...
function LibationBackupImport() {
  ...
  const [review, setReview] = useState<Record<string, MatchReview>>({});
  const [matchFilter, setMatchFilter] = useState("all");

  const inspectFiles = async (selectedFiles: FileList | File[] | null) => {
  ...
  const updateBookMatch = (group: BackupMatch, input: string) => {
  ...
  const chooseCandidate = (group: BackupMatch, candidate: BookOption) => {
    setReview((current) => ({
      ...current,
      [group.source_key]: {
  ...
  const existingAudioCount = (preview?.groups || []).filter((group) => {
    ...
    const selectedBook = selection.bookId
      ? optionById.get(selection.bookId)
      : undefined;
    return Boolean(
