...
function CharacterCard({
  ...
  const [voiceSeed, setVoiceSeed] = useState<number | string>(
    character.tts_seed ?? "",
  );
  ...
  const mutation = useMutation({
    mutationFn: (data: components["schemas"]["CharacterUpdate"]) =>
  ...
  const handleSave = () => {
    const payload: components["schemas"]["CharacterUpdate"] = {
      voice_prompt: voicePrompt || null,
        ...
        <div className="character-voice-design">
          ...
          {(character.tts_voice_provider === ttsProvider ||
            ...
            <audio
              controls
              preload="none"
              src={getCharacterVoiceSampleUrl(
                character.id,
