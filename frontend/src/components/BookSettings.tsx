...
const fetchCleanedChapters = async ({
  queryKey,
}: {
  queryKey: readonly [string, number, ...number[]];
}) => {
...
const fetchMatchedConfig = async ({
  queryKey,
}: {
  queryKey: readonly [string, number, ...number[]];
}) => {
...
function describeRevisionSnapshot(
  revision: components["schemas"]["BookRevision"],
) {
  const snapshot = revision.snapshot || {};
  if (
    revision.action === "cleaning_rules_changed" ||
    revision.action === "original_restored"
  ) {
    const removedCount = Array.isArray(snapshot.removed_chapters)
      ? snapshot.removed_chapters.length
      : 0;
    const selectorCount = Array.isArray(snapshot.content_selectors)
      ? snapshot.content_selectors.length
      : 0;
...
function BookSettings({
  ...
  const [confirmAction, setConfirmAction] = useState<{
    type: "delete" | "detach" | "original" | "revision";
    revision?: components["schemas"]["BookRevision"];
  } | null>(null);
  ...
  const saveMutation = useMutation({
    mutationFn: (data: components["schemas"]["BookUpdate"]) =>
  ...
  const restoreRevisionMutation = useMutation({
    mutationFn: (revisionId: number) =>
      restoreBookRevision(book.id, revisionId),
    onSuccess: (updatedBook) => {
      setConfirmAction(null);
      queryClient.setQueryData(["book", book.id], updatedBook);
      void queryClient.invalidateQueries({ queryKey: ["book-catalog"] });
  ...
  const toggleChapterPreview = (filename: string) => {
    ...
    >
      ...
      {bookTab === "audiobook" && (
        <AudiobookPipeline
          ...
          audiobookTab={audiobookTab ?? undefined}
        ...
        <>
            ...
            <div className="settings-row-with-cover">
              <div className="settings-fields">
                ...
                {book.metadata_details &&
                  Object.keys(book.metadata_details).length > 0 && (
                    <div className="metadata-details">
                      <span className="settings-field-label">
                        Online Details
                      </span>
                      <dl>
                        {displayValue(book.metadata_details.publisher) && (
                          <>
                            <dt>Publisher</dt>
                            <dd>
                              {displayValue(book.metadata_details.publisher)}
                        ...
                        {displayValue(book.metadata_details.published_date) && (
                          <>
                            <dt>Published</dt>
                            <dd>
                              {displayValue(
                        ...
                        {displayValue(book.metadata_details.language) && (
                          <>
                            <dt>Language</dt>
                            <dd>
                              {displayValue(book.metadata_details.language)}
                        ...
                        {displayValue(book.metadata_details.page_count) && (
                          <>
                            <dt>Pages</dt>
                            <dd>
                              {displayValue(book.metadata_details.page_count)}
                        ...
                        {displayValue(book.metadata_details.amazon_rating) && (
                          <>
                            <dt>Amazon rating</dt>
                            <dd>
                              {displayValue(
                              ...
                              {displayValue(
                                ...
                                ? ` (${displayValue(book.metadata_details.amazon_review_count)?.toLocaleString()} reviews)`
                                : ""}
                            </dd>
                          </>
                        )}
                      </dl>
                      {displayValue(book.metadata_details.description) && (
                        <p className="hint metadata-description">
                          {displayValue(book.metadata_details.description)}
                        </p>
                      )}
                    </div>
              ...
              <div className="settings-cover-aside">
                ...
                <input
                  ...
                  onChange={(e) =>
                    e.target.files?.[0] &&
                    coverMutation.mutate(e.target.files[0])
                  }
                />
                <button
                  className="btn-sm"
          ...
          {matchedConfigs.map((cfg) => (
            <section key={cfg.id} className="settings-section">
              ...
              {(cfg.chapter_selectors?.length ?? 0) > 0 && (
                <div>
              ...
              {(cfg.content_selectors?.length ?? 0) > 0 && (
                <div>
          ...
          <section className="settings-section">
            ...
            <p className="hint">
              Restore the current EPUB from its immutable original, or roll
              metadata and series fields back to a saved revision.
            </p>
            <div
              className="settings-actions"
              style={{ marginBottom: revisions.length ? "0.85rem" : 0 }}
            >
              <button
                type="button"
            ...
            {revisions.length > 0 ? (
                ...
                {revisions.map((revision) => (
                  <li className="revision-item" key={revision.id}>
                    <div>
                      ...
                      <p className="hint">
                        {describeRevisionSnapshot(revision)}
                      </p>
                    </div>
                    <button
                      type="button"
              ...
              <p className="hint">
                No reversible changes have been recorded yet.
              </p>
            )}
          </section>
          ...
          >
            ...
            {jobNotice && (
              <p className="job-queued-notice" role="status">
                {jobNotice} <a href="/activity/processing">View processing</a>
              </p>
            )}
            <p className="hint actions-hint">
              ...
              with your cleaning rules. Refreshing checks the original web
              source for new chapters.
            </p>
          ...
          >
            <p>
              The book disappears from the library, but its EPUB, cover,
              audiobook files, and history are preserved.
            </p>
            <p>
              You can restore it from Library Tools during the configured
          ...
          >
            <p>
              This replaces the current readable EPUB with the immutable
              original and clears this book’s chapter removals and content
              selectors.
            </p>
            <p>
          ...
          <ConfirmActionDialog
            ...
            onConfirm={() => {
              if (confirmAction?.revision)
                restoreRevisionMutation.mutate(confirmAction.revision.id);
            }}
          >
