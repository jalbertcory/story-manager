...
import type { ActionKind, ActionItem } from "./attention/useAttentionActions";
import {
  BookItem,
  FileItem,
  JobItem,
...
function AttentionDashboard({
  ...
  const action = (kind: ActionKind, item: ActionItem) => (
    <AttentionAction kind={kind} item={item} actions={actions} />
  );
  const bulk = (
    kind: "refresh" | "cover",
    items: (Schemas["AttentionBookItem"] | Schemas["AttentionFileItem"])[],
    label: string,
  ) => {
    const eligible = items.filter(
      (item) =>
        (kind === "cover"
          ? "can_retry_cover" in item && item.can_retry_cover
          : item.can_retry_refresh) && !actions.busy(kind, item),
    );
