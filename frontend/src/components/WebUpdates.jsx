...
import { getJson, sendWithoutBody } from "../api/client";
import { BookRow } from "./book-list/BookCards";
import SchedulerStatus from "./SchedulerStatus";

export default function WebUpdates({ onEdit }) {
  ...
  const checksQuery = useQuery({
    queryKey: ["web-checks"],
    queryFn: () => getJson("/api/library/web-checks"),
    refetchInterval: 5000,
  });
  const schedule = useQuery({
    queryKey: ["web-update-schedule"],
    queryFn: () => getJson("/api/scheduler/job"),
    refetchInterval: 10000,
  });
  const refresh = useMutation({
    mutationFn: (id) =>
      id ? refreshBook(id) : sendWithoutBody("/api/scheduler/trigger"),
    onSuccess: (_, id) => {
      setNotice(
