...
function requestedImportType() {
  const requested =
    new URLSearchParams(window.location.search).get("type") || "books";
...
const readDirEntries = (reader: FileSystemDirectoryReader) =>
  new Promise<FileSystemEntry[]>((resolve, reject) =>
    reader.readEntries(resolve, reject),
  );

const getFileFromEntry = (fileEntry: FileSystemEntry) =>
  new Promise<File>((resolve, reject) =>
    (fileEntry as FileSystemFileEntry).file(resolve, reject),
  );

async function extractEpubsFromEntries(entries: FileSystemEntry[]) {
  const readDir = async (dirEntry: FileSystemEntry): Promise<File[]> => {
...
function selectAudiobookFiles(selectedFiles: FileList | File[] | null) {
...
function scoreFilenameMatch(book: CatalogBook, fileText: string) {
...
function findFilenameBookMatch(files: File[], catalog: CatalogBook[]) {
...
const AddBook = forwardRef<
  { addFilesFromEntries: (entries: FileSystemEntry[]) => Promise<void> },
  { initialEntries?: FileSystemEntry[]; onEntriesConsumed?: () => void }
>(function AddBook({ initialEntries = [], onEntriesConsumed }, ref) {
  const queryClient = useQueryClient();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const audioInputRef = useRef<HTMLInputElement>(null);
  const audioDirectoryRef = useRef<HTMLInputElement>(null);
  ...
  const [preview, setPreview] = useState<Awaited<
    ReturnType<typeof previewBookImports>
  > | null>(null);
  ...
  const chooseAudioFiles = async (selectedFiles: FileList | File[] | null) => {
      ...
      if (importType === "books") {
        ...
        for (const item of (preview?.items || []).filter(
          ...
          try {
            ...
            completed.push({
              name: book.title || item.source_url || item.name,
              status: "queued",
              detail:
            ...
            completed.push({
              name: item.source_url || item.name,
              status: "failed",
              detail: errorMessage(error),
            });
          }
  ...
  const canImport =
    ((preview?.ready_count ?? 0) > 0 || (preview?.duplicate_count ?? 0) > 0) &&
    (preview?.duplicate_count === 0 || duplicatesReviewed);
  const selectionHeadings: Record<string, string> = {
    books: "Select book files",
    web: "Enter source URLs",
    audiobook: "Import an audiobook",
  };
  const selectionHeading = selectionHeadings[importType];
...
function BookCombobox({
  ...
  const chooseBook = (book: CatalogBook) => {
