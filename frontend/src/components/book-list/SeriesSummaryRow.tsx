...
export default function SeriesSummaryRow({
  ...
  const [editing, setEditing] = useState<"rename" | "merge" | "genres" | null>(
    null,
  ); // null | "rename" | "merge" | "genres"
  ...
  const renameMutation = useMutation({
    mutationFn: (newName: string) => renameSeries(series, newName),
    onSuccess: (_, newName) => {
  ...
  const mergeMutation = useMutation({
    mutationFn: (target: string) => mergeSeries(series, target),
    onSuccess: (_, target) => {
  ...
  const reorderMutation = useMutation({
    mutationFn: (orderedBookIds: number[]) =>
      reorderSeries(series, orderedBookIds),
    onSuccess: () => {
  ...
  const genresMutation = useMutation({
    mutationFn: (userGenreTags: string[]) =>
  ...
  const moveBook = (fromId: number | null, toId: number | null) => {
            ...
            {summary.coverBooks.length > 1 ? (
              summary.coverBooks
                ...
                .map((book, i) => (
                  <img
                    key={book.id}
                    src={getCoverUrl(book) ?? undefined}
              ...
              <img
                src={getCoverUrl(summary.coverBook) ?? undefined}
                alt={`${series} cover`}
