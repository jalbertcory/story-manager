...
import type { AudioStatus } from "./audiobook/types";
...
function AudiobookPipeline({
  ...
  const pipelineSteps = (pipelineLifecycle?.groups?.progress_steps ?? [])
  ...
  const [lastQueuedJob, setLastQueuedJob] = useState<number | boolean | null>(
    null,
  );
  ...
  const pipelineStatus = statusData?.pipeline_status ?? "";
  ...
  const doneCount = playableSentenceStatuses.reduce(
    (count, status) =>
      count + (status === null ? 0 : (sentenceCounts[status] ?? 0)),
    0,
  );
