...
import type { useBookSettingsForm } from "./useBookSettingsForm";
import { useState } from "react";

export function SelectorPills({
  selectors,
  onChange,
}: {
  selectors: string[];
...
export function BookIdentifiersSection({
  ...
  ReturnType<typeof useBookSettingsForm>,
  | "identifiersExpanded"
  | "setIdentifiersExpanded"
  | "isbn10"
  | "setIsbn10"
...
export function SourceTagList({ tags }: { tags?: string[] | null }) {
...
export function SyncedGenreTagList({ tags }: { tags?: string[] | null }) {
  if (!tags?.length) {
    return (
...
const EMPTY_UPDATE_HISTORY: components["schemas"]["BookChapterUpdateHistory"] =
  {
    book_id: 0,
    history: [],
    summary: {
...
function normalizeUpdateHistory(
  data?: components["schemas"]["BookChapterUpdateHistory"],
) {
...
function formatHistoryEntryLabel(
  entry: components["schemas"]["BookChapterUpdateHistoryPoint"],
) {
